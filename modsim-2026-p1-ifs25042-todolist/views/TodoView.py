...
class TodoView:
    ...
    def update_todo(self):
        ...
        try:
            todo_id = int(str_id)
            title = InputUtil.input("Judul Baru")
            status = InputUtil.input("Selesai? (y/n)")
            is_finished = True if status.lower() == "y" else False

            self.todo_service.update_todo(todo_id, title, is_finished)
        except ValueError:
            print("[!] ID harus berupa angka.")
    
    def search_todo(self):
        print("[Mencari Todo]")
        keyword = InputUtil.input("Kata Kunci")
    ...
    def sort_todo(self):
        ...
        if choice == "1":
            self.todo_service.sort_todo("judul")
        elif choice == "2":
